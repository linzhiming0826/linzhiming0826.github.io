...
随着业务的扩大，docker容器的增多，需要有一个类似于supervisor的管理工具，而docker-compose就是一个好的选择，今天来介绍一下基本的使用
...
## 新建yml文件
    ...
    1.一般都是建立在自己的项目目录下，直接运行 docker-compose up -d 来启动生成容器
...
## yml文件示例
    ...
    services:
        ...
        test:
            image: server:latest
            volumes:
                - ~/publish/diandian-api/:/workplace/
                - /etc/localtime:/etc/localtime
            command: python main.py -a test
            restart:
                always
...
## 命令介绍
    ...
    docker-compose up -d # 上线yml文件中配置的容器

    docker-compose down # 下线yml文件中配置的容器
    ...
    docker-compose exec *** # 进入容器交互命令

    docker-compose logs *** # 查看容器日志

    docker-compose scale test=80 # 开启80个test容器

## 命令更多

[更多](https://docs.docker.com/compose/reference/overview/)

## 注意点

    1.yml文件非常讲究缩进，所以如果出现报各种参数异常的错误，第一时间想到查看缩进是否正确
